...
try {
    ...
    $oauth = new OAuth();
    $result = $oauth->handleLinkedInCallback($code, $state);
    
    if ($result['success']) {
        // Success - redirect to homepage with success message
        $_SESSION['auth_message'] = 'LinkedIn login successful! Welcome back.';
        $_SESSION['auth_message_type'] = 'success';
        header('Location: ../index.php');
        exit;
    } else {
        // Error - redirect to login with error message
        $_SESSION['auth_message'] = 'LinkedIn login failed: ' . $result['message'];
